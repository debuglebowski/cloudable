 ...
 *                    stub at `./snapshot-port.ts`: unit 15's real `restoreSnapshot` is
 *                    person-initiated and approval-gated, which doesn't fit a system-triggered
 *                    rollback happening as part of this same transaction; reconciling that is a
 *                    real design question, not a mechanical rewire, left open for a follow-up.)
...
/**
 * A safe, human-readable description of a `createSnapshot` failure for the
 * `failureReason` field — `ArchiveDbError` and `MachineNotFoundError` don't
 * share a common shape (`.reason` vs `.machineId`), so this reads whichever
 * field is actually present rather than relying on `instanceof`/`_tag`
...
const describeSnapshotFailure = (error: ArchiveDbError | MachineNotFoundError): string => {
  ...
  return `machine ${withMachineId.machineId ?? "unknown"} not found`;
};
  ...
  Effect.gen(function* () {
    ...
    if (snapshotOutcome._tag === "Left") {
      const attempt = yield* recordAttempt({
        ...
        detail: `pre-upgrade snapshot failed: ${describeSnapshotFailure(snapshotOutcome.left)}`,
      });
      return {
        outcome: "aborted",
        machineId,
        ...
        failureReason: `pre-upgrade snapshot failed: ${describeSnapshotFailure(snapshotOutcome.left)}`,
    ...
    if (reimageOutcome._tag === "Left") {
      ...
      const reconcileOutcome = yield* Effect.either(
        provisioning.reconcile(machineId, machine.provider, machine.externalResourceId),
      );
    ...
    if (!failureReason) {
      ...
      const attempt = yield* applySuccessAtomically({
        ...
        previousConsecutiveFailures,
        now,
        preUpgradeSnapshotId: snapshot.id,
      });
    ...
    const restoreOutcome = yield* Effect.either(
      restoreSnapshot(snapshot.id, "full", {
