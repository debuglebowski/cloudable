...
describe("computeBackoff", () => {
  ...
  test("both success and failure produce a positive interval — every attempt advances the clock", () => {
    expect(
      computeBackoff({ outcome: "success", previousConsecutiveFailures: 0 }).intervalMs,
    ).toBeGreaterThan(0);
    expect(
      computeBackoff({ outcome: "failure", previousConsecutiveFailures: 0 }).intervalMs,
    ).toBeGreaterThan(0);
  });
  ...
  test("repeated consecutive failures back off exponentially — a persistently failing machine waits longer each time, not the same interval every cycle", () => {
    ...
    const second = computeBackoff({
      outcome: "failure",
      previousConsecutiveFailures: first.consecutiveFailures,
    });
    const third = computeBackoff({
      outcome: "failure",
      previousConsecutiveFailures: second.consecutiveFailures,
    });
  ...
  test("a success after a run of failures resets the streak, so the next failure backs off from the base again", () => {
    ...
    const failure2 = computeBackoff({
      outcome: "failure",
      previousConsecutiveFailures: failure1.consecutiveFailures,
    });
    const success = computeBackoff({
      outcome: "success",
      previousConsecutiveFailures: failure2.consecutiveFailures,
    });
