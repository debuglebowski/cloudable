...
/** In-memory `ApprovalService` fake — a test controls each approval's outcome via `setStatus`. */
function makeFakeApprovalService() {
  ...
  const status = (approvalId: string): Effect.Effect<ApprovalResult, Error> =>
    Effect.sync(() => ({ id: approvalId, status: statuses.get(approvalId) ?? "pending" }));

  const decide = (): Effect.Effect<ApprovalResult, Error> =>
    Effect.fail(new Error("not used by these tests"));
