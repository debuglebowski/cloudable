...
import { ArchiveError } from "../archive/archiveMachine";
...
function buildTestLayers(opts: {
        ...
        Effect.flatMap(() =>
          machineId === opts.failArchiveForMachine
            ? Effect.fail(
                new ArchiveError({
                  reason: "db_error",
                  cause: `simulated failure for ${machineId}`,
                }),
              )
            : Effect.succeed({ snapshotId: `snap-${machineId}`, retentionExpiresAt: new Date() }),
        ),
      ),
  };
  ...
  const approvalResult: ApprovalResult = { id: "approval-1", status: approvalStatus };
