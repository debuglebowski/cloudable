// Azure Instance Metadata Service (IMDS): token from IMDS, verified against
 ...
 * all ("stdlib-equivalent HTTP, one JWT library, no framework" — kept thin
 * deliberately). Reconcile at merge if unit 3's dispatcher expects a
 * different shape.
 *
 * Bare metal (the third, unimplemented `AttestationMethod`) would plug in
 * here the same way: `{ method: "bare_metal", acquireCredential }`
